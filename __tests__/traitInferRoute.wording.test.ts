...
describe('trait infer route wording', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  it('returns model-summary wording instead of trait-diagnosis wording', async () => {
    ...
    mockCreateInferTraitsUseCase.mockReturnValue({
    ...
    expect(json.message).toBe('ユーザーモデル要約を更新しました');
    expect(json.message).not.toContain('トレイト');
    expect(json.message).not.toContain('性格');
    expect(json.message).not.toContain('人格');
  });
});
